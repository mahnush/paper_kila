...
modis_22dec = '/home/mhaghigh/kilaue/modis_kil/KV_ncfile/test2_modis_kila_2km_22Dec.nc'
...
model_per_22dec = '/home/mhaghigh/kilaue/modis_kil/KV_ncfile/ncfiles_select_var/qnc_22dec_per.nc'
...
model_con_22dec = '/home/mhaghigh/kilaue/modis_kil/KV_ncfile/ncfiles_select_var/qnc_22dec.nc'
...
modis_data = [modis_22dec]#, modis_23dec]#, modis_24dec]#, modis_25dec]
...
def get_in_out(i):
    ...
    control_var_l = postpro_func.read_nc(model_con[i], var_name_model[0])[0, 60:73, :, :]*1e-6
    perturbed_var_l = postpro_func.read_nc(model_per[i], var_name_model[0])[0, 60:73, :, :]*1e-6
    control_var = np.ma.mean(control_var_l, axis = 0)
    perturbed_var =np.ma.mean( perturbed_var_l, axis = 0)

    print(np.shape(control_var))
    print(np.shape(perturbed_var))
    #msh
    modis_var = postpro_func.read_nc(modis_data[i], var_name_modis[0])
    #modis_var_ori = postpro_func.read_nc(modis_data[i], var_name_modis[0])
    #modis_cf = postpro_func.read_nc(modis_data[i], var_name_modis[4])
    #print(np.min(modis_cf))
    #print(np.max(modis_cf))
    #modis_var = np.ma.masked_where(modis_cf < 0.9, modis_var_ori)
    #print(np.min(modis_var))
    ...
    modis_inside_mask = ma.masked_where(modis_inside <=5, modis_inside)
    modis_inside_mask = modis_inside_mask.compressed()
    modis_outside_mask = ma.masked_where(modis_outside <=5, modis_outside)
...
axs0.grid(True)
axs1.grid(True)
#plt.savefig('pdf_nd_22_23Dec_limited_lat.png')
#plt.savefig('vvpdf_nd_22_23Dec_limited_lat.pdf')
